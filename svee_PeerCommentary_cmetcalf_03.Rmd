...
*C: I love your minecraft zombies, this made me chuckle!
# Setting up
...
```
*C: You can also use the summary() function which will display the mean of each variable in its overall summary of the whole dataset (although you probably already know this from the modules!), but I like how you show how someone could calculate it using the functions already in R.
...
```
*C: Your assigned objects here are very easy to identify what they are describing, nice job!
...
```
*C: it may be nice to add a title to each of your graphs, you can use the code ggtitle("name") in each of your ggplot code chunks to do this, of course I like how you labeled both axes in the way you framed your boxplots so it is still clear how the plot works. You also made me realize I forgot to include the nonbinary measurement, which I totally need to fix because that means my boxplots are not being representative of the whole population. Overall really nice looking graphs!
...
*C: It won't change your conclusion much since this spread is rather easy to detect for correlation, but if you wanted to add a line to further show this you could add the geom_smooth(method= "lm") if you would like! I took this from an example in the modules because I liked that it provided the line to show the increase correlation.
...
*C: I agree that they look relatively normal for height, weight, and age! Great observation for the tail of weight though, it does seem slightly skewed to the left with its peak, which I got the same for my histogram.
...
*C: Interesting note about it being a Poisson distribution, it makes me want to rethink which distribution I called my own! Also, perhaps you could include a brief statement about how the weight qqplot here is normal in comparison so you mentioning the weight histogram seemed skewed? I think it could make for interesting interpretations!
...
head(zombie_subset)
```
*C: Nice job citing the source that helped!
...
```
*C: The p-value seems rather small so it seems significant, which makes me want to say this looks right? I know I struggled figuring out how to calculate CI for the not normal distributions too so this will be interesting to compare with my own work.
...
```
*C: I definitely think the method above (which I also tried for my first attempt) makes it difficult to take calculations of variables within the sample, which makes me think what you did below works a lot better for what the question is asking!
