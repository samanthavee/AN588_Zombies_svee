...
# alternative method (also easier, thanks cat!)
summary(zombie)
```
...
plot_height <- ggplot(data = zombie, aes(x = gender, y = height, fill = gender)) + # calling dataset and axes
                ...
                ggtitle("Height") # adding title to the plot
...
height_age <- ggplot(data = zombie, aes(x = age, y = height)) + geom_point() +
              xlab("Age") + ylab("Height") + theme_bw() + geom_smooth(method= "lm")

weight_age <- ggplot(data = zombie, aes(x = age, y = weight)) + geom_point() +
              xlab("Age") + ylab("Weight") + theme_bw() + geom_smooth(method= "lm")

ggarrange(height_age, weight_age, ncol = 2, nrow = 1)
```
These variables look related! It looks like height and weight both increase as age increases. However, there is greater spread for the points related to weight and age. Adding a line allowed me to see the difference in slope between the two graphs; the line helps explain how the points for weight x age are more spread out than the points for height x age.
...
# Changes made after peer commentary
1. For question 1, Cat reminded me that I could use summary() as an alternative (and quicker) method for calculating the means of each variable. I added this in my chunk as a reminder. <br>
2. I added titles to my plots for question 2 - I appreciated how Cat actually provided the code that would allow me to do this. <br>
3. I added a slope line to my plots for question 3. <br>
4. For question 4, I added an extra sentence explaining the difference I noticed between the histogram and qqplot for the weight variable. <br>
Cat and I both struggled with the CIs for non-normally distributed variables in Question 5 and using a for loop to complete the first part of Question 6. We both ended up using the same alternative method for Q6 but I'm looking forward to seeing if the homework solutions will show a for loop method.
